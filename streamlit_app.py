...
def match_biplanar_cycles(front_snapshot, lateral_snapshot, sync_offset_s, tolerance_s=0.24, valid_front=None, valid_lateral=None, physical_window=None):
    """
    Versión 36 · auto-resolución de correspondencia biplanar.
    Compara hipótesis anatómica directa e inversión L/R, usa IC y fase continua,
    y conserva la solución con más pares y menor error sin ampliar tolerancias de forma indiscriminada.
    """
    A_all=_cycles_from_snapshot(front_snapshot); B_all=_cycles_from_snapshot(lateral_snapshot)
    coarse=float(sync_offset_s)
    phase_adj,phase_corr=_phase_alignment_fine_offset(front_snapshot,lateral_snapshot,coarse)

    if not A_all or not B_all:
        return [],"No calculable",{"coarse_offset_s":coarse,"fine_correction_s":np.nan,"final_offset_s":coarse,
            "phase_correction_s":phase_adj,"phase_correlation":phase_corr,"lr_hypothesis":"No calculable",
            "direct_pairs":0,"swapped_pairs":0,"mae_s":np.nan}

    # Hipótesis de offset derivadas de IC, sin asumir todavía L/R.
    offsets=[coarse]
    if np.isfinite(phase_adj): offsets.append(coarse+float(phase_adj))
    for a in A_all:
        for b in B_all:
            implied=float(b["ic_s"]-a["ic_s"])
            if abs(implied-coarse)<=0.60: offsets.append(implied)
    # deduplicar a resolución de 10 ms
    offsets=sorted(set(round(float(x),2) for x in offsets))
    ...
    for off in offsets:
        ...
        if physical_window is not None:
            ...
            B=[b for b in B if _cycle_overlaps_physical_window(b,lfps,t0,t1,float(off))]
        for swapped in (False,True):
            pairs,mae,dur,score=_biplanar_pair_hypothesis(A,B,off,swapped,tolerance_s)
            # prioridad lexicográfica: nº pares, error IC, coherencia duración, proximidad al coarse
            rank=(len(pairs), -(mae if np.isfinite(mae) else 9), -(dur if np.isfinite(dur) else 9), -abs(off-coarse))
            if best is None or rank>best["rank"]:
                best={"offset":float(off),"pairs":pairs,"mae":mae,"dur":dur,"swapped":swapped,"rank":rank}

    pairs=best["pairs"] if best else []
    final_offset=best["offset"] if best else coarse
    fine=final_offset-coarse
    mae=best["mae"] if best else np.nan
    # diagnóstico de ambas hipótesis en el offset final
    A=list(A_all); B=list(B_all)
    if physical_window is not None:
        t0,t1=map(float,physical_window); ffps=float(front_snapshot.get("fps",0)); lfps=float(lateral_snapshot.get("fps",0))
        A=[a for a in A if _cycle_overlaps_physical_window(a,ffps,t0,t1,0.0)]
        B=[b for b in B if _cycle_overlaps_physical_window(b,lfps,t0,t1,final_offset)]
    pdirect,md,_,_= _biplanar_pair_hypothesis(A,B,final_offset,False,tolerance_s)
    pswap,ms,_,_= _biplanar_pair_hypothesis(A,B,final_offset,True,tolerance_s)
    hyp="L↔R invertido" if best and best["swapped"] else "L↔L / R↔R directo"
    if not pairs: quality="No calculable"
    else: quality="Alta" if len(pairs)>=3 and mae<=0.06 else ("Moderada" if len(pairs)>=2 and mae<=0.12 else "Baja")
    diag={"coarse_offset_s":coarse,"fine_correction_s":float(fine),"final_offset_s":float(final_offset),
          "phase_correction_s":phase_adj,"phase_correlation":phase_corr,"lr_hypothesis":hyp,
          "direct_pairs":len(pdirect),"swapped_pairs":len(pswap),"direct_mae_s":md,"swapped_mae_s":ms,"mae_s":mae}
...
with tabs[3]:
                ...
                if st.button("Calcular ambas vistas, guardar histórico y eliminar vídeos", type="primary", use_container_width=True):
                    try:
                        ...
                        metrics += [
                            ...
                            {"key":"biplanar_sync_fine_correction_s","label":"Corrección fina por ciclos IC (Initial Contact / Contacto Inicial)","value":float(pair_diag.get("fine_correction_s",np.nan)),"unit":"s","quality":pair_q,"notes":"Versión 35: ajuste robusto usando eventos IC del mismo lado y duración de ciclo."},
                            {"key":"biplanar_sync_final_offset_s","label":"Desfase temporal final frontal-lateral","value":float(pair_diag.get("final_offset_s",np.nan)),"unit":"s","quality":pair_q,"notes":"Desfase global más corrección fina; se utiliza para emparejar ciclos."},
                            {"key":"biplanar_phase_correction_s","label":"Corrección por fase continua de marcha","value":float(pair_diag.get("phase_correction_s",np.nan)),"unit":"s","quality":"Control interno","notes":"Versión 36: segunda referencia temporal basada en correlación de señal distal continua; no crea ciclos por sí sola."},
                            {"key":"biplanar_phase_correlation","label":"Correlación de fase continua frontal-lateral","value":float(pair_diag.get("phase_correlation",np.nan)),"unit":"r","quality":"Control interno","notes":"Coherencia temporal independiente de los eventos IC (Initial Contact / Contacto Inicial)."},
                        ...
                        st.session_state.biplanar_sync_offset_s = float(sync_user)
    ...
    if st.session_state.get("metrics_done"):
        ...
        if st.session_state.get("mode", "").startswith("2 cámaras"):
            ...
            st.markdown("##### Correspondencia temporal frontal-lateral · Versión 35")
            s1,s2,s3=st.columns(3)
            s1.metric("Desfase global inicial", fmt(metric_value(metrics,"biplanar_sync_coarse_offset_s"),3)+" s")
            s2.metric("Corrección fina por IC (Initial Contact / Contacto Inicial)", fmt(metric_value(metrics,"biplanar_sync_fine_correction_s"),3)+" s")
            s3.metric("Desfase final", fmt(metric_value(metrics,"biplanar_sync_final_offset_s"),3)+" s")
            ...
            h3.markdown(_hyp)
            p1,p2=st.columns(2)
            p1.metric("Corrección por fase continua", fmt(metric_value(metrics,"biplanar_phase_correction_s"),3)+" s")
            p2.metric("Correlación de fase continua", fmt(metric_value(metrics,"biplanar_phase_correlation"),3))
            ...
            if _ff>0:
                for _side,_obj in [("I",_fs),("D",_fr)]:
                    if _obj:
                        for _x in np.asarray(_obj.get("ic",[]),dtype=float):
